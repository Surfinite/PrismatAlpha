...
No mods, no SWF patches — just a network proxy that speaks the same protocol as the game.

Requirements: Python 3.8+ (no third-party packages needed)

How it works:
  1. Temporarily redirects Prismata's server hostname to 127.0.0.1 (via hosts file)
  2. Runs a local TCP proxy that forwards traffic to the real server
  3. Once you log in, it asks the server for your replay codes
  4. Saves them to a text file (one code per line)
  5. Restores the hosts file when done
...
def main():
    ...
    print("Setting up network redirect...")
    ...
    if success:
        ...
        print(f"Total: {sum(1 for _ in open(output_path))} codes")
    else:
