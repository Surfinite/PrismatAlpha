#!/bin/bash
# Launch GPU EC2 instance for DeepSets neural network training
# Usage: bash aws/launch_deepsets_training.sh [OPTIONS]
#
# Uploads pre-vectorized HDF5 files (~4GB) and trains with streaming mode
# (chunk-buffered reads, minimal RAM usage — handles any dataset size).
#
...
#   LABEL=deepsets_12M   Run label for S3 output dir (default: auto)
...
#   training/data/fleet_v3.h5          — DeepSets V2 format (~5.9M records)
#   training/data/fleet_v4.h5          — DeepSets V2 format (~5.9M records)
#   training/data/local_mbvmb.h5       — DeepSets V2 format (~414K records)
...
echo "=== Prismata DeepSets Training GPU Launch ==="
...
if [ "${SKIP_UPLOAD:-false}" = "true" ]; then
  ...
  for f in "${TRAIN_FILES[@]}"; do
    basename=$(basename "$f")
    echo "    $basename..."
    aws s3 cp "$f" "s3://$BUCKET/deploy/deepsets/$basename" --region $REGION
  done
...
echo "=== Prismata DeepSets Training Worker Starting ==="
echo "Run ID: \$RUN_ID"
echo "Label: \$LABEL"
...
# HDF5 multi-process safety: avoid file locking issues with gzip-compressed chunks
...
aws s3 cp s3://\$BUCKET/deploy/deepsets/fleet_v3.h5 \$WORK/data/fleet_v3.h5 --region \$REGION
aws s3 cp s3://\$BUCKET/deploy/deepsets/fleet_v4.h5 \$WORK/data/fleet_v4.h5 --region \$REGION
aws s3 cp s3://\$BUCKET/deploy/deepsets/local_mbvmb.h5 \$WORK/data/local_mbvmb.h5 --region \$REGION
aws s3 cp s3://\$BUCKET/deploy/deepsets/human_1500_no6s_v2.h5 \$WORK/data/human_1500_no6s_v2.h5 --region \$REGION
...
for f in ['data/fleet_v3.h5', 'data/fleet_v4.h5', 'data/local_mbvmb.h5', 'data/human_1500_no6s_v2.h5']:
    with h5py.File(f'\$WORK/' + f, 'r') as h:
        total += h['label_A'].shape[0]
print(total)
")
echo "Dataset: \$TOTAL_N total records across 4 files"
...
echo ""
echo "=== DeepSets Training complete ==="
...
USERDATA_FILE="c:/libraries/PrismataAI/aws/.userdata_deepsets_tmp.sh"
...
if [ "$USE_SPOT" = "true" ]; then
  ...
  SPEC_FILE="c:/libraries/PrismataAI/aws/.spot_spec_deepsets_tmp.json"
...
echo ""
echo "=== DeepSets Training Instance Launched ==="
...
echo "  2. Download pre-vectorized HDF5 data from S3 (~2 min)"
