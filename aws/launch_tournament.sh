#!/bin/bash
# Launch EC2 Windows instance for tournament evaluation
# Usage: bash aws/launch_tournament.sh [INSTANCE_TYPE] [NUM_ROUNDS] [VM_MULTIPLIER]
#
# Examples:
#   bash aws/launch_tournament.sh                        # t3.micro, 1500 rounds, 1x
#   bash aws/launch_tournament.sh c5.xlarge 2000 1.3     # c5.xlarge, 2000 rounds, 1.3x think
#
# Runs NeuralTest (UCT) + NeuralAB_vs_Original (AB) against OriginalHardestAI.
# Each process runs both tournaments sequentially. Multiple processes = more games.
...
echo "  Rounds:   $NUM_ROUNDS (per process, per tournament)"
...
TOTAL_GAMES=$(( NUM_ROUNDS * 2 * PROCESSES * 2 ))  # rounds * 2(color swap) * processes * 2(tournaments)
echo "  Processes:     $PROCESSES (4 threads each)"
echo "  Think time:    7000ms x ${VM_MULTIPLIER} = ${TIME_LIMIT_MS}ms"
echo "  Total games:   ~$TOTAL_GAMES (${NUM_ROUNDS}r x 2 x ${PROCESSES}p x 2t)"
echo ""
...
$bucket = "prismata-selfplay-data"
$runId = "eval_" + (Get-Date -Format "yyyy-MM-dd_HH-mm-ss")
...
Read-S3Object -BucketName $bucket -Key "deploy/asset/config/neural_weights.bin" -File "C:\eval\asset\config\neural_weights.bin"
...
# Patch config line-by-line: disable all, enable NeuralTest + NeuralAB_vs_Original
$lines = $config -split "`n"
for ($i = 0; $i -lt $lines.Length; $i++) {
    if ($lines[$i] -match '"NeuralTest"' -and $lines[$i] -notmatch 'NeuralTestFast') {
        ...
        Write-Host "Enabled NeuralTest on line $i"
    } elseif ($lines[$i] -match '"NeuralAB_vs_Original"') {
        $lines[$i] = $lines[$i] -replace '"run"\s*:\s*false', '"run":true'
        $lines[$i] = $lines[$i] -replace '"run"\s*:\s*true', '"run":true'
        $lines[$i] = $lines[$i] -replace '"rounds"\s*:\s*\d+', "`"rounds`":$numRounds"
        if ($lines[$i] -notmatch '"Threads"') {
...
$config = $config -replace '("PrismatAlpha_UCT"\s*:\s*\{[^}]*"TimeLimit"\s*:\s*)\d+', "`${1}$timeLimitMs"
...
$debugConfig -split "`n" | ForEach-Object {
    if ($_ -match 'NeuralTest|NeuralAB_vs_Original|PrismatAlpha_UCT|PrismatAlpha_AB_Legacy|OriginalHardestAI') {
        Write-Host "DEBUG: $_"
    }
}
...
function Sync-EvalToS3 {
    ...
    # Replay directories
    $replayDirs = Get-ChildItem "C:\eval\asset\replays\*" -Directory -ErrorAction SilentlyContinue
    foreach ($dir in $replayDirs) {
        $tempReplayDir = "$tempBase\replays\$($dir.Name)"
        New-Item -ItemType Directory -Force -Path $tempReplayDir | Out-Null
        $files = Get-ChildItem $dir.FullName -File -ErrorAction SilentlyContinue
        foreach ($f in $files) {
            try {
                Copy-Item $f.FullName "$tempReplayDir\$($f.Name)" -Force
                Write-S3Object -BucketName $bucket -Key "eval-results/$runId/replays/$($dir.Name)/$($f.Name)" -File "$tempReplayDir\$($f.Name)"
                $syncCount++
            } catch { Write-Host "[Sync] Warning: $($f.Name): $_" }
        }
    }
...
INSTANCE_ID=$(aws ec2 run-instances \
  ...
  $SPOT_OPTS \
  --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=PrismataEval-$NUM_ROUNDS}]" \
...
echo "  Rounds:      $NUM_ROUNDS x $PROCESSES processes = $(( NUM_ROUNDS * PROCESSES )) per tournament"
...
echo "  4. Run tournaments (NeuralTest + NeuralAB_vs_Original)"
echo "  5. Upload HTML results + replays to s3://$BUCKET/eval-results/"
...
echo "  aws s3 sync s3://$BUCKET/eval-results/ bin/eval-results/ --region $REGION"
