# PrismatAI Training Plan — Version 1
**Date:** 2026-03-07
**Status:** FINAL
...
- **V4 — JS engine accuracy:** The JavaScript engine is a transpilation of the original AS3 engine and is considered authoritative. It has been validated against replay data (100% pass rate on a sample of 500 replays). During Phase 2a extraction, every replay will be validated as it passes through the engine. Any failures will be logged and excluded.

- **V5 — MCDSAI as success benchmark:** The target for supervised training is: the trained model, used as the evaluation function in Alpha-Beta HPS search, wins a statistically significant majority of games against MCDSAI. MCDSAI's strength relative to LiveHardestAI (our strongest C++ reference) has been established by a 512-game LiveHardestAI vs MCDSAI matchup: **MCDSAI wins approximately 75%** in both ResourcesDD and ResourcesCC configurations. This confirms MCDSAI is meaningfully stronger than LiveHardestAI and is a suitable benchmark target. The same runs serve as V11 click-failure verification. MasterBot (the in-game Steam AI) would be the most meaningful real-world reference point but requires Steam move injection, which is currently unfinished and may not be feasible. MasterBot is excluded from the plan as a blocking requirement; it can be added later if injection is completed.
...
- **V11 — C++ AI <-> JS engine click protocol correctness (hard gate for Phase 5 and Phase 6):** This is distinct from V4 (JS engine rule correctness). V4 verifies that the JS engine implements game rules correctly by stepping through replays. V11 verifies that the C++ Alpha-Beta AI can successfully communicate its chosen moves to the JS engine through the click protocol — i.e., that the click sequences the C++ AI generates are correctly interpreted, executed without failures, and produce the move the AI intended.
  ...
  **Verification protocol:** The 512-game LiveHardestAI vs MCDSAI matchup (Phase 0b) serves as the V11 verification run. Before Phase 5 or Phase 6 begins, confirm from those results:
...
### 0b. Benchmark Tournament (DONE)
- **512-game LiveHardestAI vs MCDSAI matchup completed.** MCDSAI wins approximately **75%** in both ResourcesDD and ResourcesCC configurations. This confirms MCDSAI is meaningfully stronger than LiveHardestAI.
- Human expert visual review of a sample of games should confirm strategic coherence (moves look sensible, no obviously broken game states).
- **Result:** MCDSAI is a suitable and challenging benchmark. Its ~75% win rate against LiveHardestAI means the neural model must achieve substantial strength gains to beat it.
...
- **Status:** Treat as a future experiment if Options 1 and 2 plateau. Not the starting point for iteration 1.
...
- **Status:** A promising medium-term direction, but not the starting point. Best pursued in parallel with Phase 3 ablations once basic results are in.
...
- **Implementation:** Use `torch.nn.BCEWithLogitsLoss` (not `BCELoss` after manual sigmoid). The `WithLogits` variant is numerically stable for soft targets and avoids log(0) when predictions are near 0 or 1. <!-- CHANGED: Specified BCEWithLogitsLoss — Reviewer 4 -->
...
- **Mirror augmentation correctness test:** For a sample of 1,000 records, verify:
  ...
  - `label_original + label_mirror == 1.0` **exactly** (labels invert correctly) — this must hold for all three label strategies including Strategy C (Elo-interpolated). For Strategy C, verify algebraically: if `label = (1-t)*elo_prior + t*outcome`, then `mirror_label = (1-t)*(1-elo_prior) + t*(1-outcome) = 1 - label`. <!-- CHANGED: Strengthened mirror test from approximate to exact; added Strategy C coverage — Reviewer 5 -->
...
- **Gradient clipping:** Apply gradient clipping (max norm 1.0) as a default for all training runs. This prevents occasional exploding-gradient training collapses when the model is confident but wrong (predicted 0.95, label is 0.0), which is common with BCE loss and noisy binary labels. Never harmful, cheap insurance. <!-- CHANGED: Added gradient clipping — Reviewers 4, 5 -->
- **Stochastic Weight Averaging (SWA):** Maintain an SWA model alongside the standard checkpoint — average weights from the last ~20% of epochs. SWA often produces smoother, more robust models with noisy labels. PyTorch provides `torch.optim.swa_utils`. Start SWA collection relatively late — starting too early while loss is still rapidly descending washes out the weights. Compare SWA model against best single checkpoint in Phase 5. <!-- CHANGED: Added SWA timing guidance — Reviewers 3, 4 -->
...
**Learning rate warmup:** Use a short linear warmup (500-2000 steps) before reaching peak LR. This prevents early training instability with Adam + BCE on noisy labels, especially if gradient clipping is relaxed. Standard practice, cheap insurance. <!-- CHANGED: Added LR warmup — Reviewers 4, 5 -->
...
- Think time: **3 seconds per turn** (consistent with Phase 0b benchmark). **Also report results at 1s and 7s think times** — if the neural player's advantage increases with think time, the evaluator is good but needs search depth to express it. If it decreases, the evaluator may be brittle under deeper search (finding states that expose evaluation errors). <!-- CHANGED: Added multi-think-time evaluation — Reviewers 5, 6 -->
- Report Wilson confidence interval, not just point estimate. **Use paired statistical tests** (McNemar's test or sign test on pair outcomes) rather than simple binomial, since matches are paired by card set. This substantially increases statistical power. <!-- CHANGED: Specified paired statistical tests — Reviewers 5, 6 -->
...
- **Card-set stratified evaluation:** Report tournament win rate separately for card sets that appear in training data vs. card sets not seen during training. This detects memorization vs. generalization. **Also report separately for card sets containing low-frequency units** (flagged in Phase 0d) — the model's evaluation of rare-unit positions is essentially untrained. <!-- CHANGED: Added rare-unit stratified reporting — Reviewers 5, 6 -->
...
### 5b-2. Offline Search-Position Sanity Check (Before Tournaments) <!-- CHANGED: Added intermediate offline gate — Reviewer 6 -->
...
### 5b-3. Search Integration Ablation <!-- CHANGED: Added search interaction ablations — Reviewer 6 -->
...
**Recommended action:** Extract a data-driven opening book as a standalone Phase 0 task (before Phase 5), independent of the neural training schedule. **Every primary tournament result in Phase 5 should report whether the opening book was active.** Run at least one evaluation with and one without the expanded book. If the win rate difference is large (>5pp), the model may be partially dependent on the book to funnel play into familiar positions rather than evaluating well generally. <!-- CHANGED: Elevated opening book timing and with/without comparison — Reviewers 3, 5, 6 -->
...
The following questions were open in the original draft and have been resolved through expert review.

1. **Feature richness:** Option 2 (type-count + instance flags + supply + card-set indicator) is the starting point. Option 1 (Churchill-exact) runs as a Phase 3a baseline. Option 2's instance-level flags (building, tapped, damaged) encode information that directly affects tactical evaluation — "3 Rhinos, 2 tapped" is strategically very different from "3 Rhinos, 0 tapped." Supply remaining and in-card-set flag are essential. All already implemented in C++.

2. **Rating floor:** Keep the 1500 floor with rating-based sample weighting (Phase 4a). V8 data shows only 0.2% of games (~230) fall in the 1500-1599 bracket — raising the floor gains almost nothing and loses real data. Weighting is strictly better.

3. **Card set filtering:** Exclude base-only games. Base-only Prismata is a strategically distinct subgame with no relevance to competitive play (which always includes random units). Measure count in Phase 0d; if 500+ exist, use for a side experiment.

4. **Value target:** Binary win outcome with soft-label modification (Phase 1b). A graded resource-advantage target has problems: it requires defining "resource advantage" in a unit-composition-dependent way, and games involving strategic sacrifice would be mislabelled. Win prediction is correct — soft-labelling fixes the noise issue.

5. **Architecture prior:** Residual (skip) connections in an MLP are nearly free and help at depth >=3. Full ResNets are designed for spatial/grid inputs — Prismata's state doesn't have that structure. Residual MLP blocks allow going deeper without degradation and are trivially portable to C++ (infrastructure already exists in `NeuralNet.cpp`). Plain MLP is fine at 2-3 layers; residual preferred at 4+.

6. **MCDSAI calibration:** Resolved. Phase 0b tournament complete: MCDSAI wins ~75% vs LiveHardestAI in both ResourcesDD and ResourcesCC configurations. MCDSAI is confirmed as a meaningful and challenging benchmark. Beating a fixed AI is necessary but not sufficient — supplement with held-out prediction accuracy and move agreement metrics (Phase 5c). Also run MCDSAI vs. itself at different think times (1s vs 7s) to see how much search depth matters for MCDSAI specifically.
...
The following items were suggested by reviewers and have merit but are not critical. They are listed here for selection — tell Claude which numbers to apply.

| # | Enhancement | Reviewer(s) | Effort | Recommendation |
|---|---|---|---|---|
| 1 | **Option 1.5 intermediate feature set** — type counts + constructing counts + supply + in_card_set + globals (~500 features). Test as a middle ground between Option 1 and Option 2 | R6 | Small | Lean no — Option 1 vs 2 ablation sufficient |
| 2 | **Pairwise/ranking auxiliary loss** — within same game, winning-player states should rank above losing-player states. Improves search ordering | R6 | Medium | Lean yes for iteration 2 |
| 3 | **Value monotonicity probes** — add sanity tests: same state + extra resources shouldn't reduce value; same state + removed opponent blockers shouldn't reduce value | R6 | Small | Lean yes |
| 4 | **W&B/MLflow for ablation tracking** — with 15+ ablation configurations, spreadsheet tracking becomes error-prone | R5 | Small | Lean yes |
| 5 | **Per-game weight normalization** — cap each game's total contribution to prevent long games from dominating the dataset | R6 | Small | Neutral — may conflict with temporal weighting |
| 6 | **Noise injection** — add small Gaussian noise (sigma=0.05) to normalized feature vector as alternative regularization to Mixup | R5 | Trivial | Neutral |
| 7 | **Stratify test by card-set novelty** — report separately for: exact 8-unit sets seen in training, unseen sets but seen units, rare-unit-heavy sets | R6 | Small | Lean yes |
| 8 | **Endgame oversampling** — if turn 25+ positions are <10% of training data, oversample them 2-3x | R5 | Small | Neutral |
| 9 | **Define primary offline metric upfront** — state explicitly whether BCE, Brier score, or calibration error is the primary offline metric | R6 | Trivial | Lean yes |
| 10 | **Compute budget estimates per phase** — estimate hours/cost for extraction, ablations, full training, tournaments | R6 | Small | Lean yes |
| 11 | **Heuristic+net blend as explicit Phase 5 ablation** — test NeuralNetPlusPlayout blend alongside pure neural; the C++ infrastructure already supports this | R6 | Trivial | Lean yes |
| 12 | **Fix V-numbering gap** — V6 and V9 are missing from the verification items; renumber for clean presentation | R6 | Trivial | Lean yes |
| 13 | **Turn-bucketed label entropy** — in Phase 0d, report label entropy by turn bucket to quantify noise level at different game phases | R5, R6 | Small | Lean yes |
| 14 | **Schema hash in binary shard headers** — embed version checksum in shard headers (not just filenames) to prevent accidental mixing | R5 | Small | Lean yes |
| 15 | **P1 win rate by rating bracket** — in Phase 0d, compute first-player win rate conditioned on rating bracket; if P1 advantage varies with skill level, the Elo prior may need to be rating-dependent | R5 | Small | Lean yes |
