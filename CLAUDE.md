...
**Next actions:**
1. **Run B (R12 256h/3L) training on GCP — IN PROGRESS.** Instance `prismata-training-100010` (g2-standard-8, L4, ~$0.70/hr from credits). Step 147K, epoch 5, **85.0% val acc** (project record), val_loss=0.3107, brier=0.0816, still improving. Patience=25 evals (125K steps). Config: lr=2e-5, dropout=0.20, label_smooth=0.90, batch_size=512, streaming. S3 log: `s3://prismata-selfplay-data/training-runs/full726k_R12_256h3L/2026-02-20_10-00-57/`. Run A (256h/2L) weights exported: `neural_weights_run_A_256h2L.bin`. Run C (512h/2L) terminated — inferior. **Next: when Run B finishes, download best model, export weights, launch tournament eval (8K+ games vs OriginalHardestAI).**
...
- **Partial recovery (Feb 20)**: Errored replays now emit examples from turns *before* the error instead of discarding. Recovered 2,653 examples from 121 failed replays. `partialRecovered` counter in summary output.
...
- **GameState private methods**: `beginPhase()`, `endPhase()`, `initFromJSON()`, `runScript()` are all PRIVATE. External code must use public API: `beginTurn()`, `doAction()`, `addCard()`, `setMana()`, etc. `beginTurn(player)` is the public equivalent of `beginPhase(Swoosh)`.
- **`getClickAction(card)` exists** (`GameState.cpp:2197-2269`): Maps a Card to the correct Action for the current phase/context. Handles all 13 action types (USE_ABILITY, UNDO_USE_ABILITY, SELL, SNIPE, CHILL, UNDO_CHILL, ASSIGN_BLOCKER, ASSIGN_BREACH, UNDO_BREACH, ASSIGN_FRONTLINE, WIPEOUT, plus targeting). Always use this instead of reimplementing click-to-action logic.
- **`SelfPlayDataSink` constructor requires `std::atomic<uint32_t>&`**: Caller must own the atomic game counter with longer lifetime than the sink. Also requires `threadIndex` (use different values per process to avoid shard filename collisions).
- **Engine layer architecture**: `source/engine/` has zero AI or testing imports. Layered: `engine/ → ai/ → testing/`. New classes that depend on NeuralNet or SelfPlayDataSink belong in `testing/`, not `engine/`.
...
- **Click counting ≠ buy counting (CRITICAL)**: `card clicked` in commandList does NOT guarantee a successful purchase. Clicks on sold-out cards (legendary supply exhausted, etc.) are recorded but silently rejected by the game engine. `revert clicked` only undoes intentional undos, not failed clicks. **Any code parsing buys from clicks MUST enforce supply limits**: legendary = 1 per player, rare ≈ 4. Without this, buy counts are inflated — e.g., Mega Drone (legendary) showed 3x purchased from click data when only 1 is possible. **Sniffer now enforces supply limits (Feb 20)**: `Session.supply_limits` built from `mergedDeck` rarity at BeginGame, `Session.game_buys` tracks cumulative purchases per card_id, clicks exceeding limits rejected. Revert handling decrements counts.
...
- **Undo snipe IMPLEMENTED (Feb 20)**: `UNDO_USE_ABILITY` now works for snipe cards. Three changes: (1) SNIPE doAction tracks kill via `addKilledCardID`, (2) `undoTargetAbility` has SNIPE case (resurrection via `killedCardIDs` mechanism), (3) `isLegal` no longer blocks undo for snipe cards. SnipeGreedyKnapsack already used undo-targeting-mode; this enables undoing completed snipes.
- **UntapAvoidBreach zero-value optimization (Feb 20)**: When total attack < enemy defense (can't breach), untaps all attacking blockers to use for defense instead. Previously only untapped drones via `untapCardsAvoidBreach` heuristic.
- **Blocking feature mismatch FIXED (Feb 20)**: `vectorize.py` now matches C++ — uses `blocking` alone for slot 3 (was `blocking AND abilityUsed`, misclassifying blockers that hadn't used abilities as "ready").
...
- **claude-mem bun daemon uses ~600MB RAM**: `worker-service.cjs --daemon` runs persistently. Normal but significant on 32GB system under memory pressure.
...
| `docs/plans/2026-02-20-work-items.md` | Non-training work items (23 items, prioritized, with status tracking) |
...
| `docs/plans/2026-02-20-cpp-replay-mode.md` | C++ ReplayStepper plan — click→action mapping, --replay and --analyze CLI modes (3 phases) |
| `docs/plans/2026-02-20-cpp-replay-mode-context.md` | Standalone context doc for external review of C++ replay mode plan |
...
**Replay search API**: `POST https://prismata-stats.web.app/api/search/replays` (form-encoded: `lower_date`, `upper_date`, `lower_rating`, `replay_rated`). Needs `ssl.CERT_NONE` in Python. `prismata.net` SSL cert expired. ~36,000 expert replays synced as of Feb 20, 2026. Expert data not useful for current value-only self-play training (27M records dwarfs it), but valuable later for policy head training and iterative RL.
...
| FlopFlop collection | 5,000 | `bin/ReplayCodes_FlopFlop.txt` |
