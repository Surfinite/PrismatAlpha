...
**512h comparison training IN PROGRESS (Feb 18).** Running locally on XPU with `--num-workers 2` (reduced from 4 to avoid RAM thrashing). Output: `training/models/512h_330k/`. At epoch 5, step 55000, 85.1% val accuracy. Estimated ~8-10 hours total.
...
**Next actions:**
1. **512h comparison training running overnight** — output at `training/models/512h_330k/`. When done: export weights, deploy to S3, run tournament eval on 24 EC2 instances to compare 256h vs 512h at 330K games.
...
- **Prismata client architecture**: Adobe AIR/Flash app. C++ engine compiled to AVM2 bytecode via CrossBridge. Memory reading infeasible — use clipboard or network proxy for live state access.
...
| `tools/prismata_sniffer.py` | TCP proxy for Prismata AMF3 protocol (ports 11600/11601) |
| `tools/prismata_advisor.py` | Python overlay — clipboard monitor + C++ --suggest + tkinter always-on-top display |
