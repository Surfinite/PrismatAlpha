...
- **matchup_clean.js auto end-swipe applies to ALL AIs**: `StateUtil.js` previously gated auto end-swipe behind `&& daveAI`. This has been removed — without it, stale BREACH swipes block OVERKILL clicks on under-construction units for C++ `--suggest` players. If re-adding daveAI gating, restore the guard and also add explicit end-swipe for non-daveAI players.
- **matchup_clean.js confirm→defense auto-commit**: C++ AI emits one space click for action→confirm transition, but JS engine requires two clicks (confirm→commit→defense). `matchup_clean.js` auto-inserts the commit click when it detects confirm phase with incoming defense clicks. Without this, defense blocker clicks execute as UNDO in confirm phase.
- **Don't add LiveHardestAI resignation until click verification (V11) is complete**: Early resignation hides click failures — if the AI resigns before failed clicks would have occurred, they never appear in logs. Verify zero `skippedBuys` and zero click failures across 500+ games before adding resignation logic.
- **matchup log false positives**: Scanning logs for "fail"/"skip" keywords produces thousands of false positives — "Infusion Grid" card name, "0 failed" success messages, PowerShell metadata. Filter for actual failures: `grep -E "[1-9][0-9]* failed"` rather than bare keyword grep.
...
- **`build_replay_db.py --source X` wipes the DB**: Full build (no `--incremental`) deletes the existing DB before importing only that one file — all start_time, ratings, and player metadata is lost. Always use `--incremental --source balance_results.json` when applying balance validation results to an existing DB.
...
- **Replay balance validation**: `validate_db_codes.py` checks both buyCost AND rarity against `cardLibrary.jso` (rarity mismatch = wrong supply: legendary=1, rare=4, normal=20). DB state (Mar 7, 2026 — full re-validation complete): 203,602 codes validated, 154,061 `balance_passed=1`, 48,707 failed, 834 fetch errors. Training set: 102,697 games (rated, 1500+, balance-passed). To re-validate after cardLibrary changes: `python reset_for_rarity_revalidation.py` → `python validate_db_codes.py` → `python build_replay_db.py --incremental --source balance_results.json`.
- **Revalidation is destructive — backup first**: `reset_for_rarity_revalidation.py` removes entries from `balance_results.json` AND resets `balance_validated`/`balance_passed` flags in the DB. If re-validation fails partway (network issues, script crash), the previous validation state for tens of thousands of codes is lost. Always backup `replays.db` and `balance_results.json` before running. A previous context lost data this way and had to restore from backups.
...
| `c:\libraries\prismata-replay-parser\replays.db` | SQLite replay database (203K codes, 154,061 balance-passed, full re-validation Mar 7 2026) |
...
| `c:\libraries\prismata-replay-parser\validate_db_codes.py` | Balance validator — checks buyCost + rarity vs cardLibrary.jso, writes `balance_results.json` |
| `c:\libraries\prismata-replay-parser\audit_rarity_changes.py` | Rarity change auditor — Phase 1 monthly sample, Phase 2 narrow window, Phase 4 impact |
| `c:\libraries\prismata-replay-parser\reset_for_rarity_revalidation.py` | Resets balance_validated=0 for pre-May-2019 passing codes before re-validation |
| `c:\libraries\prismata-replay-parser\rarity_audit_results.json` | Raw Phase 1 audit findings (6,308 samples) |
| `c:\libraries\prismata-replay-parser\rarity_change_windows.json` | Per-unit rarity change transition windows |
...
| `docs/plans/2026-03-06-training-plan-v1-draft.md` | Authoritative V1 training plan — supervised training through self-play, with verification gates V1–V11 |
