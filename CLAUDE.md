...
- **`--suggest` clicks array (Feb 20)**: Output now includes `"clicks":[{_type,_id},...]` — wire-protocol-ready sequence for protocol injection. BUY: `_type="card clicked"`, `_id=mergedDeck index` (CardType ID - 2). USE_ABILITY/ASSIGN_BLOCKER/BREACH: `_type="inst clicked"`, `_id=client instId`. SNIPE/CHILL: two clicks (source then target). END_PHASE: `_type="space clicked"`, `_id=-1`. Automatic END_PHASE insertion mirrors `Move::toClientString()` logic. **Trailing clicks must include TWO "space clicked"** (enter confirm + commit turn), matching `Move::toClientString()` (Move.cpp:107-108).
- **`--suggest` click application**: `matchup_main.js` does NOT check `canClick` return value from `analyzer.recordClick()` — failed clicks are silently skipped. Add verbose logging when debugging click issues. `suggestClicksToClicks()` in suggest_adapter.js filters out "end swipe processed" entries.
- **`buildInitDeck` preserves activeDeck order**: `initDeck[0..N-1]` = `activeDeck[0..N-1]` (same cards, same indices). Inactive cards appended after. Card IDs from C++ `--suggest` (mergedDeck index) correctly map to JS State.cards indices for active cards.
...
- **Replay state debugging**: Check `turn` and `phase` fields across consecutive replay states to detect turn-completion bugs. Same `turn` value in consecutive states = turn didn't commit. Quick check: `python -c "import json; d=json.load(open('replay.json')); [print(f'state {i}: turn={s[\"turn\"]} phase={s[\"phase\"]}') for i,s in enumerate(d['states'])]"`.
- **DetailedReplays per-action stepping**: Tournament config `"DetailedReplays": true` saves one state snapshot per action (not per turn). Implementation copies pre-turn state, replays each action from `getPreviousMove()`, and serializes intermediate states. GUI replay viewer (arrow keys) then steps through individual actions. Replay JSON includes `"detailedReplays": true` flag.
