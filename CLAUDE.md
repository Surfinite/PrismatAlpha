...
9. **Build live AI commentator** — PLANNED (Feb 20). Claude Haiku + edge-tts + OBS WebSocket for Twitch-ready game casting. Sniffer provides real-time events, Haiku generates commentary (~$0.03/game), edge-tts speaks it. Plan: `docs/plans/2026-02-20-live-commentator-plan.md`. Knowledge extraction instructions: `docs/plans/2026-02-20-commentary-knowledge-extraction.md`. Output: `docs/commentary-knowledge/`.
...
- **`_sanitize_gamestate()` in both sniffer and advisor**: Intentional duplication — the tools are independent (sniffer is a network proxy, advisor is a clipboard overlay). Both need to parse F6 clipboard JSON. Do not refactor into a shared module.
...
- **Batch validation**: 2,127 Master Bot replays tested (Feb 20). Pass rate: 56.6% (1,203/2,127) after RC#7+RC#8 fixes. Remaining 924 failures: SNIPE overcounting (38%), USE_ABILITY (26%), END_PHASE (14%), BUY/OTHER (18%), BLOCKER (4%). Not blocking self-play.
...
- **Live commentator deps** (not yet installed): `anthropic` (Claude API), `edge-tts` (neural TTS, async), `sounddevice` + `pydub` (audio playback/decode), `obsws-python` (OBS WebSocket). External: VB-Cable (virtual audio for OBS routing). See `docs/plans/2026-02-20-live-commentator-plan.md`.
...
- **TS tooling bugs (FIXED, validation improved)** — RC#5 (snipe target), RC#6 (frontline→breach), RC#7 (two-step targeting: USE_ABILITY before SNIPE/CHILL), RC#8 (action ordering: abilities→snipe→frontline→buy), selfsac/lifespan tolerance all fixed. Pass rate 27.2%→56.6% (1,203/2,127). Remaining failures: SNIPE overcounting in TS parser, USE_ABILITY/END_PHASE resolution differences. Not blocking self-play.
...
| `training/convert_replay_for_cpp.py` | Convert TS replay states to C++ validation format |
...
| `aws/launch_training.sh` | EC2 GPU training launcher (g4dn.xlarge, Linux, env var config, auto-terminate) |
...
| `gcp/launch_training.sh` | GCP GPU training launcher (L4/T4, Linux DL VM, env var config, auto-delete) |
...
| `run_prismata_tools.bat` | Combined launcher — sniffer proxy + advisor overlay (parallel start) |
| `tools/prismata_commentator.py` | Live AI commentator — sniffer events → Claude Haiku → edge-tts → OBS (PLANNED, not yet created) |
...
| `docs/commentary-knowledge/` | Extracted Prismata strategy knowledge for commentator system prompt (7 KB files + README + sources + handoff) |
