...
- **`--analyze` CLI mode** (DONE Feb 21): `Prismata_Testing_d.exe --analyze replay.json --player PrismatAlpha_AB --think-time 500` — extends `--eval` with full AI search at each position. Compares human buys (from commandList clicks) vs AI buys (from `Player::getMove()`). Outputs per-turn agreement, sorted buy lists, AI full move. ~300ms/turn at 200ms think. Human buy extraction doesn't enforce supply limits (click overcounting possible for legendaries).
