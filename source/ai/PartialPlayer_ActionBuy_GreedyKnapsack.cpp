...
void PartialPlayer_ActionBuy_GreedyKnapsack::getMove(GameState & state, Move & move)
{
    ...
    AITools::SortVector(_buyableTypes, BuyKnapsackCompare(_heuristic, state, _playerID, _enemyAttackPotential, _frontlinePenalty));
...
bool PartialPlayer_ActionBuy_GreedyKnapsack::canAffordToActivate(const CardType cardType, const GameState & state) const
{
    ...
    // determine our income for the next turn
    ...
    if (_legacy)
    {
        // legacy: cumulative check across all owned units with colored ability costs
    ...
    {
        // improved: check if income covers THIS card's activation cost only
        if (!beginTurnIncome.has(abilityCost))
        {
            return false;
        }
    }
