...
void GameState::initFromJSON(const rapidjson::Value & value)
{
    ...
    for (rapidjson::Value::ConstMemberIterator itr = value.MemberBegin(); itr != value.MemberEnd(); ++itr)
    {
        ...
        else if (prop == "glassBroken")
        {
            // JS uses glassBroken flag within PHASE_ACTION to represent breach sub-state
            // C++ uses an explicit Phases::Breach enum value
...
void GameState::endPhase()
{
    ...
    switch (m_activePhase)
    {
        ...
        case Phases::Confirm:
        {
            ...
            // check game-over BEFORE incrementing turn number (matches JS MOVE_COMMIT)
            ...
            // only increment turn number if game is not over (matches JS: ++numTurns after win check)
            m_turnNumber++;
...
std::string GameState::toJSONString() const
{
    ...
    // JS uses glassBroken flag within "action" phase for breach sub-state
    // C++ uses explicit Phases::Breach enum; serialize as "action" + glassBroken for JS compat
    if (getActivePhase() == Phases::Breach)
    {
        ss << "\"phase\":\"action\", \n";
        ss << "\"glassBroken\":true, \n";
